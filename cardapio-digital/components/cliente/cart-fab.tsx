...
export default function CartFab({ isOpen = true }: Props) {
  ...
  if (!isOpen) {
    return (
      <div className="fixed bottom-4 left-0 right-0 px-4 z-50 max-w-md mx-auto">
        <button
          onClick={() => toast.error('Restaurante fechado no momento. Volte mais tarde!')}
          className="w-full flex items-center justify-between bg-gray-500 text-white pl-4 pr-5 py-3.5 rounded-2xl shadow-2xl shadow-gray-500/40 active:scale-[0.98] transition-transform"
        >
          <div className="flex items-center gap-3">
            <div className="relative h-10 w-10 rounded-xl bg-white/20 flex items-center justify-center">
              <ShoppingBag className="h-5 w-5" strokeWidth={2.5} />
              <span className="absolute -top-1.5 -right-1.5 bg-white text-gray-500 text-[11px] rounded-full h-5 min-w-5 px-1 flex items-center justify-center font-bold shadow-md">
                {count}
              </span>
            </div>
            <div className="leading-tight">
              <p className="text-[11px] font-medium text-white/85">Restaurante fechado</p>
